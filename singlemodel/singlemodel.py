...
class ImageProcessor(threading.Thread):
    def __init__(self, image_paths,ocr,result_queue,formatted_time):
        super().__init__()
...
def convert(folder_path,formatted_time):
    ...
    image_lists = [image_paths[i:i+len(image_paths)//4] for i in range(0, len(image_paths), len(image_paths)//4)]
    # 创建 5 个线程来处理图片
    threads = [ImageProcessor(image_list,ocr,result_queue,formatted_time) for image_list in image_lists]
    # 启动线程并等待完成
